...
D3DApp::D3DApp(HINSTANCE hInstance, std::string winCaption, D3DDEVTYPE devType, DWORD reqVertPrcs)
{
	mhInstance = hInstance;
	mMainWndCaption = winCaption;
	mDevType = devType;
	mReqVertPrcs = reqVertPrcs;
	mhMainWnd = 0;
	md3dObject = 0;
...
void D3DApp::initMainWindow()
{
	...
	wc.lpszMenuName = 0;
	wc.lpszClassName = "D3DWndPractice";
	...
	mhMainWnd = CreateWindow("D3DWndPractice", mMainWndCaption.c_str(), WS_EX_STATICEDGE, 100, 100, R.right, R.bottom, 0, 0, mhInstance, 0);
...
void D3DApp::initDirect3D()
{
	...
	HR(md3dObject->CreateDevice(D3DADAPTER_DEFAULT, mDevType, mhMainWnd, devBehaviorFlags, &md3dPP, &gd3dDevice));
}
        ...
        {	
			...
			if( !isDeviceLost() )
			{
				...
				if (dt > 1.0f) dt = 0.0f;
				 ...
				 //for debug purposes - Speeding it up is fun :D

				frameLimit +=dt;
				if (frameLimit > 0.001350f) // 0.0167 -> 60
				{
					updateScene(frameLimit);
					drawScene();
					frameLimit = 0.0f;
				}
	...
	/*case WM_SIZE:
		if( gd3dDevice )
		{
			...
			else if( wParam == SIZE_RESTORED )
			{
				...
				if( mbMinimized && md3dPP.Windowed )
				{
					onLostDevice();
...
void D3DApp::enableFullScreenMode(bool enable)
{
	...
	else
	{
		...
		// SetWindowPos for the change to take effect.  In addition, we 
		// need to call this function anyway to update the window dimensions.
		SetWindowPos(mhMainWnd, HWND_BOTTOM, 100, 100, R.right, R.bottom, SWP_SHOWWINDOW);
	}
